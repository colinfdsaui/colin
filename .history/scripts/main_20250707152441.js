...
$(function () {
  ...
  // start in “single”‑page look
$fb.addClass('hide-spread');

// when the user moves off page 1, show the spread
$fb.bind('turned', function(e, page) {
  if (page > 1) {
    $fb
      .removeClass('hide-spread')
      .addClass('show-spread');
  } else {
    // back to page 1? hide the empty side again
    $fb
      .removeClass('show-spread')
      .addClass('hide-spread');
  }
});
...
// Zoom on click of the page’s <img>
$fb.on('click', '.page img', function(e) {
  e.stopPropagation();            // don’t bubble up to flipbook
  const $img = $(this);
